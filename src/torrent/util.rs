...
pub async fn get_torrent_hash(torrent_meta: &TorrentMeta,
                              download_meta_provider: &impl DownloadMetaProvider) -> Result<String, String> {
    if torrent_meta.magnet_url.is_some() {
        Ok(MagnetLink::new(torrent_meta.magnet_url.as_ref().unwrap())
            .map_err(|err| err.to_string())?
            .hash()
            .to_string())
    } else if torrent_meta.download_url.is_some() {
        match download_meta_provider.get_download_meta(torrent_meta.download_url.as_ref().unwrap()).await {
            Ok(content) => {
                match content {
        ...
        Err(format!("Neither magnet nor download link exist for torrent {}", torrent_meta))
    }
}
